...
export function installTabletop({renderer,scene,root,sky,ground,grid,controls,getCamera,span,floors,setFloor,heatmap,prepare,restore}){
 ...
 let tab='floors',activeFloor='all',floorPage=0,follow=true,menuOpen=false,snapPanel=true,palmUp=false,touchLatch=false,lastTouch=-Infinity,lastTime=0;
 ...
 function renderPanel(){clearPanel();
  const cv=document.createElement('canvas');cv.width=960;cv.height=1120;const c=cv.getContext('2d');c.fillStyle='#edf2f4';rounded(c,0,0,960,1120,44);c.fillStyle='#13313d';c.textAlign='left';c.font='bold 45px sans-serif';c.fillText('TwinXR',52,78);c.font='23px sans-serif';c.fillStyle='#617986';c.fillText('BUILDING EXPLORER',52,115);c.fillStyle='#218876';c.beginPath();c.arc(875,74,9,0,Math.PI*2);c.fill();c.font='21px sans-serif';c.fillStyle='#627b85';c.fillText('Touch a card or point + pinch',52,1060);c.fillText('Left palm: menu · Pin: keep panel here',52,1090);
  ...
  ['floors','model','data'].forEach((t,i)=>card(t==='floors'?'Floors':t==='model'?'Model':'Sensors',()=>{tab=t;renderPanel();},(i-1)*.197,-.07,.184,.048,{active:tab===t}));
  if(tab==='floors'){
  ...
  card(follow?'Pin panel':'Follow me',()=>{follow=!follow;snapPanel=follow;renderPanel();},-.195,-.605,.181,.053,{active:!follow});card('Hide',()=>toggleMenu(false),0,-.605,.181,.053);card('Exit AR',()=>session?.end(),.195,-.605,.181,.053,{danger:true});
 }
 renderPanel();
 ...
 function toggleMenu(value=!menuOpen){menuOpen=value;panel.visible=value;if(value&&follow)snapPanel=true;drag=null;}
 ...
 function updateHandMenu(frame,ref,pose){
  ...
  if(menuOpen&&(snapPanel||(follow&&!nearPanel&&!touchLatch))){const dir=new T.Vector3(0,0,-1).applyQuaternion(headOrientation);dir.y=0;if(dir.lengthSq()<.01)dir.set(0,0,-1);dir.normalize();const right=new T.Vector3(-dir.z,0,dir.x);const target=headPosition.clone().addScaledVector(dir,.67).addScaledVector(right,-.34);target.y=headPosition.y+.08;
   panel.position.lerp(target,snapPanel?1:1-Math.exp(-dt*5));panel.lookAt(headPosition.x,panel.position.y,headPosition.z);snapPanel=false;
  }
